...
for (let i = 0; i < stories.length; i++) {
  ...
  const chapterTitle = story.chapter || "";
  let chapterDiv = chapters.get(chapterTitle);

  if (!chapterDiv) {
    chapterDiv = document.createElement('div');
    chapterDiv.classList.add('chapter');
    chapterDiv.innerHTML = `<h2 class="chapter-title">${chapterTitle}</h2>`;
    chapterContainer.appendChild(chapterDiv);

    chapters.set(chapterTitle, chapterDiv);
  }
  ...
  if (story.speakers) {
    innerHTML += `<h3 class="story-speakers"><span class="story-id"></span>${story.speakers}</h3>`
  }

  innerHTML += `
    <div class="audio-player">
      <img class="audio-button" src="./images/play.png" />
      <span class=audio-text>Abspielen</span>
  ...
  function onPlayToggle() {
    ...
    if (currentlyPlaying !== story) {
      ...
      audio.src = `audio/${story.audiofile}`;
